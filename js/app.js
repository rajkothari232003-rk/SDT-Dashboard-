...
function removeAccountCompletely(i){
  const acc = MASTER_EDIT.accounts[i];
  if (!acc) return;
  const ok = confirm('Remove account ' + acc + ' completely? This deletes its trades, positions, pending alerts, funds and alert history. Future webhook alerts for this account will be ignored after deploy.');
  if (!ok) return;
  const typed = prompt('Type DELETE ' + acc + ' to confirm.');
  if (typed !== 'DELETE ' + acc) return;
  clearErr();
  google.script.run
    .withSuccessHandler(res => {
...
function saveDaySnapshot(btn){
  clearErr();
  const ok = confirm("Save today's P&L snapshot? This is only for records and will not change trades or next-day P&L.");
  if (!ok) return;
  btn.disabled = true;
  const old = btn.textContent;
  btn.textContent = 'Saving...';
  google.script.run
    .withSuccessHandler(res => {
      btn.disabled = false;
      btn.textContent = old;
      const out = document.getElementById('daySnapshotSaved');
      if (out) {
        out.textContent = 'Saved ' + (res.date || 'today') + ': ' +
          (res.accounts || 0) + ' account row(s), ' + (res.rows || 0) + ' P&L row(s).';
        out.hidden = false;
        setTimeout(() => out.hidden = true, 7000);
      }
    })
    .withFailureHandler(e => {
      btn.disabled = false;
      btn.textContent = old;
      showErr('Could not save day snapshot: ' + (e && e.message ? e.message : e));
    })
